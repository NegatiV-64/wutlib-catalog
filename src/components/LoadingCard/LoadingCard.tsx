...
export const LoadingCard: FC<LoadingCardProps> = ({ className }) => {
  return (
    <article className={cn('flex flex-col pt-3 pb-5 px-6 bg-white rounded-lg overflow-hidden shadow-xl th:w-10/12 th:mx-auto', styles.card, className)}>
      ...
      <div className={cn('mb-2 h-6', styles.text, styles.skeleton)}></div>
      <div className={cn('mb-2 h-6', styles.text, styles.skeleton)}></div>
      <div className={cn('w-28 h-11 mt-auto th:block th:mx-auto', styles.text, styles.skeleton)}></div>
    </article>
  );
};